 ...
 *   ⑤ ★ প্রতিটি কার্ডের কোণায় অন/অফ টগল-সুইচ — ক্লিকেই PATCH টগল, কোনো রিলোড/পপ-আপ
 *      ছাড়াই সাথে-সাথে হোমপেজে কার্ড দেখা/লুকানো যায় (একদম অন-অফ সুইচের মতো)
...
export default function HomeLeadershipPanel() {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoadError(false)
    try {
      const slotRes = await fetch('/api/admin/home-leadership')
      if (slotRes.ok) {
  ...
  /* ─── ★ তাৎক্ষণিক অন/অফ টগল — অপটিমিস্টিক আপডেট + PATCH; কোনো পপ-আপ/রিলোড নেই ─── */
  const handleToggleVisibility = async (slotKey: string, nextState: boolean) => {
    setSlots((prev) =>
      prev.map((s) => (s.slotKey === slotKey ? { ...s, isActive: nextState } : s))
    )
    setToggleBusy(slotKey)
    try {
      const res = await fetch('/api/admin/home-leadership', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ slotKey, isActive: nextState }),
      })
      if (res.ok) {
        ...
        setSlots((prev) =>
          prev.map((s) =>
            s.slotKey === slotKey ? { ...s, isActive: Boolean(saved.isActive) } : s
          )
        )
      } else {
  ...
  const renderSlotCard = (slot: SlotData) => {
    ...
    return (
      <div
        ...
        } ${!slot.isActive && !isEditing ? 'opacity-60' : ''}`}
      >
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <ToggleWithLabel
              on={slot.isActive}
              disabled={toggleBusy === slot.slotKey}
              onClick={() => handleToggleVisibility(slot.slotKey, !slot.isActive)}
            />
          </div>
        </div>
  ...
  const visibleCount = slots.filter((s) => s.name && s.isActive).length
