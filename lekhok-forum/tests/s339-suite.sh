...
#      (ovRows330-৯-সারি — hr343-W-সারি-সহ); কীবোর্ড-parity hr343-এ সম্পন্ন (দ্বি-চাপ-আর্ম/ওয়াইপ-রীতি-অটুট — নথিভুক্তকরণ-সম্পন্ন);
...
grep -q "q333h.register('W'" "$EJSF" && bad "home-reorder.ejs: register-'W'-কল-ঘটেছে (স্ট্যাটিক-সারি-রীতি-ভাঙা)" || ok "home-reorder.ejs: register-কল-শূন্য (স্ট্যাটিক-সারি-রীতি — hr343-সম-রীতি)"
grep -q "\['W', 'স্থায়ী-হিন্ট-স্টোর মুছুন (দ্বি-চাপ-নিশ্চিত)', 'W = স্থায়ী মুছুন'\]" "$EJSF" && ok "home-reorder.ejs: W-সারি-নথিভুক্ত (ovRows330-৯-সারি — hr343-parity — সুইট-বিবর্তন-জুটি-সম্পন্ন)" || bad "home-reorder.ejs: W-সারি-অনুপস্থিত (hr343-parity-চুক্তি-ভাঙা)"
...
LAJ=$(unjj "$LA")
if [ "$(jf btn "$LAJ")" = "true" ] && printf '%s' "$(jf lbl "$LAJ")" | grep -q 'স্থায়ী মুছুন' && printf '%s' "$(jf ar "$LAJ")" | grep -q 'দ্বি-চাপ-নিশ্চিত' && [ "$(jf st "$LAJ")" = "9" ] && [ "$(jf sv "$LAJ")" -ge 1 ] && printf '%s' "$(jf last "$LAJ")" | grep -q 'sync'; then ok "ই২ই: জীবন্ত-উপস্থিতি (পাবলিক-setHint → persist337-মোড়ক → বাটন-রি-রেন্ডার-বিহীন-তাৎক্ষণিক-উপস্থিত + গেট-লেবেল/aria-টেমপ্লেট + স্ন্যাপশট-৯-কী — hr337-সর্ব-রেজিস্ট্রি-অর্থবিদ্যা)"; else bad "ই২ই: $(unjj "$LA")"; fi
...
RBJ=$(unjj "$RB")
if [ "$(jf btn "$RBJ")" = "true" ] && [ "$(jf st "$RBJ")" = "9" ]; then ok "ই২ই: ব্যাচ-স্থায়ীকরণ-পরবর্তী-পুনঃউপস্থিতি (setHints → persist-মোড়ক → বাটন-তাৎক্ষণিক — জীবন্ত-সিঙ্ক-দ্বৈত-প্রমাণ + স্ন্যাপশট-৯-কী)"; else bad "ই২ই: $(unjj "$RB")"; fi
...
CPJ=$(unjj "$CP")
if [ "$(jf btn "$CPJ")" = "true" ] && printf '%s' "$(jf ho "$CPJ")" | grep -q 'নির্মাণ-পথ' && [ "$(jf cls37 "$CPJ")" = "true" ] && [ "$(jf st "$CPJ")" = "9" ]; then ok "ই২ই: নির্মাণ-পথ (স্থায়ী-স্টোর-সহ-রিলোড → replay → বাটন-সৃষ্টিকালীন-উপস্থিত + hr337-restored-অটুট + স্ন্যাপশট-৯-কী — s337-উত্তরাধিকার-সহাবস্থান)"; else bad "ই২ই: $(unjj "$CP")"; fi
...
REGJ=$(unjj "$REG")
if [ "$(jf r "$REGJ")" = "9" ]; then ok "ই২ই: রেজিস্ট্রি-৯-সারি-অটুট (hr343-W-সারি-সহ — ওভারলে-সারি-গণনা-স্থায়িত্ব)"; else bad "ই২ই: $(unjj "$REG")"; fi
