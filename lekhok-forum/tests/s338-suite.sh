...
GHJ=$(unjj "$GH")
if [ "$(jf n "$GHJ")" = "8" ] && [ "$(jf x "$GHJ")" = "true" ] && [ "$(jf cp "$GHJ")" = "true" ] && [ "$(jf g "$GHJ")" = "2" ] && printf '%s' "$(jf last "$GHJ")" | grep -q 'get:'; then ok "ই২ই: getHints-রিড-API (৮-কী-RAW + ডিফল্ট-মিল + **কপি-সুরক্ষা** {MUTATED-বাহ্যিক-মিউটেশন-অস্পৃশ্য} + gets=২ + get:-last)"; else bad "ই২ই: $(unjj "$GH")"; fi
...
RTJ=$(unjj "$RT")
if [ "$(jf dc "$RTJ")" = "1" ] && [ "$(jf r "$RTJ")" = "8" ] && [ "$(jf cls38 "$RTJ")" = "false" ] && [ "$(jf cls36 "$RTJ")" = "true" ] && [ "$(jf al "$RTJ")" = "0.15" ] && [ "$(jf x "$RTJ")" = "true" ] && [ "$(jf pv "$RTJ")" = "true" ]; then ok "ই২ই: রাউন্ড-ট্রিপ (getHints→সম্পাদনা→diff{changed:১}→setHints{applied:৮} → গেট-অফ + hr336-batched-১৫% + RAW-আপডেট + hr337-স্থায়ীকরণ-স্বয়ংক্রিয়)"; else bad "ই২ই: $(unjj "$RT")"; fi
...
APJ=$(unjj "$AP")
if [ "$(jf c "$APJ")" = "0" ] && [ "$(jf s "$APJ")" = "8" ] && [ "$(jf t "$APJ")" = "8" ] && [ "$(jf mi "$APJ")" = "0" ] && [ "$(jf cls "$APJ")" = "false" ]; then ok "ই২ই: রাউন্ড-ট্রিপ-স্বয়ংসমতা (diff(getHints()) = {changed:০, same:৮, missed:০} — রিড-নিরপেক্ষতা-প্রমাণ)"; else bad "ই২ই: $(unjj "$AP")"; fi
rld
...
OVJ=$(unjj "$OV")
if [ "$(jf o "$OVJ")" = "true" ] && [ "$(jf li "$OVJ")" = "8" ] && [ "$(jf r "$OVJ")" = "8" ]; then ok "ই২ই: ওভারলে-৮-সারি (রিড-API-রো-গণনা-অস্পৃশ্য-প্রমাণ)"; else bad "ই২ই: $(unjj "$OV")"; fi
