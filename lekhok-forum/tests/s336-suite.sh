...
APJ=$(unjj "$AP")
if [ "$(jf reg "$APJ")" = "true" ] && [ "$(jf r0 "$APJ")" = "9" ] && [ "$(jf r1 "$APJ")" = "10" ] && [ "$(jf a "$APJ")" = "1" ] && [ "$(jf a334 "$APJ")" = "1" ] && [ "$(jf a335 "$APJ")" = "1" ] && [ "$(jf kb "$APJ")" = "true" ] && [ "$(jf eq "$APJ")" = "true" ]; then ok "ই২ই: হিন্ট-বিহীন-কী-ব্যাচ = অ্যাপেন্ড-পথ (rows ৯→১০ + appends-দ্বি-গণনা-ডেল্টা×১ + লাইভ-উপস্থিত + live==line)"; else bad "ই২ই: $(unjj "$AP")"; fi
...
OVJ=$(unjj "$OV")
if [ "$(jf o "$OVJ")" = "true" ] && [ "$(jf li "$OVJ")" = "10" ] && [ "$(jf r "$OVJ")" = "10" ]; then ok "ই২ই: ওভারলে-১০-সারি (৯+W + J336-নিবন্ধিত — ব্যাচ-রো-গণনা-অস্পৃশ্য — পুনঃনির্মাণ-শূন্য-প্রমাণ)"; else bad "ই২ই: $(unjj "$OV")"; fi
