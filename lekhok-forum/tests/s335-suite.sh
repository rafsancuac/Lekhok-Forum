...
MSJ=$(unjj "$MS")
if [ "$(jf m "$MSJ")" = "false" ] && [ "$(jf mi "$MSJ")" = "1" ] && [ "$(jf r "$MSJ")" = "8" ]; then ok "ই২ই: অজানা-কী-সেট = false (misses=১ + rows-অপরিবর্তিত)"; else bad "ই২ই: $(unjj "$MS")"; fi
...
APJ=$(unjj "$AP")
if [ "$(jf reg "$APJ")" = "true" ] && [ "$(jf ok4 "$APJ")" = "true" ] && [ "$(jf r0 "$APJ")" = "8" ] && [ "$(jf r1 "$APJ")" = "9" ] && [ "$(jf a335 "$APJ")" = "1" ] && [ "$(jf kb "$APJ")" = "true" ] && [ "$(jf eq "$APJ")" = "true" ]; then ok "ই২ই: হিন্ট-বিহীন-কী-সেট = অ্যাপেন্ড-পথ (rows ৮→৯ + appends=১ + লাইভ-উপস্থিত + live==line)"; else bad "ই২ই: $(unjj "$AP")"; fi
...
OVJ=$(unjj "$OV")
if [ "$(jf o "$OVJ")" = "true" ] && [ "$(jf li "$OVJ")" = "9" ] && [ "$(jf r "$OVJ")" = "9" ]; then ok "ই২ই: ওভারলে-৯-সারি (J335-নিবন্ধিত — setHint-রো-গণনা-অস্পৃশ্য — পুনঃনির্মাণ-শূন্য-প্রমাণ)"; else bad "ই২ই: $(unjj "$OV")"; fi
