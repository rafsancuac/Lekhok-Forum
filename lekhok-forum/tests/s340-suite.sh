...
#     (পয়েন্টার-কেবল — hr339-উত্তরাধিকার — ওভারলে-৯-সারি — hr343-বিবর্তন)।
...
RRJ=$(unjj "$RR")
if printf '%s' "$(jf rawF "$RRJ")" | grep -q '{fmt}' && printf '%s' "$(jf resF "$RRJ")" | grep -q 'সমৃদ্ধ' && [ "$(jf n "$RRJ")" = "1" ] && printf '%s' "$(jf last "$RRJ")" | grep -q 'getr:9' && [ "$(jf rows "$RRJ")" = "9" ] && [ "$(jf ss "$RRJ")" = "true" ] && [ "$(jf rb "$RRJ")" = "false" ]; then ok "ই২ই: RAW-vs-রেজলভড (raw F='{fmt}'-অরেজলভড + resolved F='সমৃদ্ধ'-কল-টাইম + getr:৯-কী + রিড-পবিত্রতা {স্টোরেজ-শূন্য + রো=৯ + বাটন-অস্পৃশ্য})"; else bad "ই২ই: $(unjj "$RR")"; fi
...
LSYJ=$(unjj "$LSY")
if [ "$(jf rb "$LSYJ")" = "true" ] && [ "$(jf cb "$LSYJ")" = "true" ] && [ "$(jf b2 "$LSYJ")" = "0" ] && [ "$(jf st "$LSYJ")" = "9" ] && printf '%s' "$(jf last "$LSYJ")" | grep -q 'sync:on' && printf '%s' "$(jf al "$LSYJ")" | grep -q 'getHints(true)'; then ok "ই২ই: জীবন্ত-সিঙ্ক-অন (persist337-মোড়ক → বাটন-রি-রেন্ডার-বিহীন-তাৎক্ষণিক-উপস্থিত + hr339-ক্লিয়ার-সহাবস্থান + শূন্য-ইতিহাসে .hr324-btn=০-স্বতন্ত্র-শ্রেণি-প্রমাণ + aria-getHints(true)-টেমপ্লেট)"; else bad "ই২ই: $(unjj "$LSY")"; fi
...
CP1J=$(unjj "$CP1")
if [ "$(jf capF "$CP1J")" = "true" ] && [ "$(jf capN "$CP1J")" -ge 30 ] 2>/dev/null && [ "$(jf c "$CP1J")" = "1" ] && printf '%s' "$(jf last "$CP1J")" | grep -q 'copy:9' && [ "$(jf done "$CP1J")" = "true" ] && [ "$(jf h1 "$CP1J")" = "$H0" ]; then ok "ই২ই: রেজলভড-রপ্তাই-কপি (ক্লিপবোর্ড-ক্যাপ-রেজলভড-JSON {সমৃদ্ধ-সহ} + copies=১ + copy:৯-মার্কার + .hr340-done-ফ্ল্যাশ + ইতিহাস-গণনা-অপরিবর্তিত ($H0) — রেকর্ড-বিহীন-পথ-প্রমাণ)"; else bad "ই২ই: $(unjj "$CP1") h0=$H0"; fi
