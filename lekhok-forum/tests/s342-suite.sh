...
B1J=$(unjj "$B1")
if printf '%s' "$(jf jF "$B1J")" | grep -q 'JSON' && printf '%s' "$(jf rF "$B1J")" | grep -q 'সমৃদ্ধ' && printf '%s' "$(jf rawF "$B1J")" | grep -q '{fmt}' && [ "$(jf keys "$B1J")" = "2" ] && [ "$(jf g "$B1J")" = "1" ] && printf '%s' "$(jf last "$B1J")" | grep -q 'getmb:2' && [ "$(jf rows "$B1J")" = "9" ] && [ "$(jf ss "$B1J")" = "true" ] && [ "$(jf bb "$B1J")" = "false" ] && [ "$(jf raws "$B1J")" = "1" ] && [ "$(jf getr "$B1J")" = "0" ]; then ok "ই২ই: মাল্টি-মোড-ব্যাচ (getHints(['json','rich']) = জুটি {json-F='JSON' + rich-F='সমৃদ্ধ' — mode326='rich'-তেও} + RAW='{fmt}'-অটুট + getmb:১ + getmb:2-মার্কার + রিড-পবিত্রতা {রো=৯ + স্টোরেজ-শূন্য} + এক-RAW-স্ন্যাপশট {raws=১ — দ্বি-মোডে-এক-ফেচ} + getr=০-অস্পৃশ্য)"; else bad "ই২ই: $(unjj "$B1")"; fi
...
LSYJ=$(unjj "$LSY")
if [ "$(jf bb "$LSYJ")" = "true" ] && [ "$(jf ms "$LSYJ")" = "true" ] && [ "$(jf mb "$LSYJ")" = "true" ] && [ "$(jf rb "$LSYJ")" = "true" ] && [ "$(jf cb "$LSYJ")" = "true" ] && [ "$(jf b2 "$LSYJ")" = "0" ] && [ "$(jf st "$LSYJ")" = "9" ] && printf '%s' "$(jf last "$LSYJ")" | grep -q 'sync:on' && printf '%s' "$(jf brCls "$LSYJ")" | grep -q 'hr324-bar hr342-bar'; then ok "ই২ই: জীবন্ত-সিঙ্ক-অন (persist337-মোড়ক → বাটন-রি-রেন্ডার-বিহীন-তাৎক্ষণিক-উপস্থিত + hr339/hr340/hr341-সহাবস্থান + .hr324-btn=২-গণনা-চুক্তি-অটুট + স্বতন্ত্র-বার hr324-bar hr342-bar + store=৯)"; else bad "ই২ই: $(unjj "$LSY")"; fi
