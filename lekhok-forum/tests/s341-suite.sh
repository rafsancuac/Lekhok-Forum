...
MXJ=$(unjj "$MX")
if printf '%s' "$(jf jF "$MXJ")" | grep -q 'JSON' && printf '%s' "$(jf riF "$MXJ")" | grep -q 'সমৃদ্ধ' && printf '%s' "$(jf rawF "$MXJ")" | grep -q '{fmt}' && printf '%s' "$(jf curF "$MXJ")" | grep -q 'সমৃদ্ধ' && [ "$(jf getm "$MXJ")" = "2" ] && printf '%s' "$(jf last "$MXJ")" | grep -q 'getm:rich' && [ "$(jf rows "$MXJ")" = "8" ] && [ "$(jf ss "$MXJ")" = "true" ] && [ "$(jf ms "$MXJ")" = "false" ] && [ "$(jf raws340 "$MXJ")" = "2" ] && [ "$(jf getr340 "$MXJ")" = "1" ]; then ok "ই২ই: মোড-নির্দিষ্ট-রেজলিউশন (getHints('json')-F='JSON' + getHints('rich')-F='সমৃদ্ধ' — mode326='rich'-তেও + RAW='{fmt}'-অটুট + getm:২ + রিড-পবিত্রতা {রো=৮ + স্টোরেজ-শূন্য} + এক-উৎস-কাউন্টার {raws=২ {২-মোড-স্ন্যাপশট} + getr=১ — raw() = QA-হুক-বাইপাস-ডক-কৃত})"; else bad "ই২ই: $(unjj "$MX")"; fi
