#!/bin/bash
# s296-regression.sh — session296 পূর্ণ-রিগ্রেশন রানার (s260..s296 + role-policy + guard:design + audit:views)
...
SUITES=(s260-eventsfilter s261-resfilter s262-mufilter s263-mdfilter s264-adfilter s265-acfilter \
        ...
        s290-comboad s291-railcap s292-filmthumb s293-dcfilter s294-cejump s295-stripkeys s296-epkwide)
if [ "${#SUITES[@]}" != "38" ]; then echo "FATAL: তালিকা-মোট ${#SUITES[@]} (৩৮-প্রত্যাশিত)"; exit 1; fi
case "${SUITES[*]}" in *s295-stripkeys*) ;; *) echo "FATAL: রানারে s295-stripkeys-অনুপস্থিত"; exit 1;; esac
case "${SUITES[*]}" in *s294-cejump*) ;; *) echo "FATAL: রানারে s294-cejump-অনুপস্থিত"; exit 1;; esac
case "${SUITES[*]}" in *s293-dcfilter*) ;; *) echo "FATAL: রানারে s293-dcfilter-অনুপস্থিত"; exit 1;; esac
FROM="${CHUNK_FROM:-1}"
TO="${CHUNK_TO:-38}"
if ! [ "$FROM" -ge 1 ] 2>/dev/null || ! [ "$TO" -le 38 ] 2>/dev/null || [ "$FROM" -gt "$TO" ]; then echo "FATAL: চাঙ্ক-সীমা-অবৈধ ($FROM..$TO)"; exit 1; fi
TOTAL=0; BAD=()
...
if [ "${#BAD[@]}" = "0" ]; then echo "══ রিগ্রেশন ($TOTAL-সুইট): সব-সবুজ ══"; exit 0; else echo "══ রিগ্রেশন: ফেল=${BAD[*]} ══"; exit 1; fi
