#!/bin/bash
# s291-regression.sh — session291 পূর্ণ-রিগ্রেশন রানার (s260..s291 + role-policy + guard:design + audit:views)
# রীতি: প্রতি-সুইট-পরে ফল-সারাংশ; কোনো-এক-সুইট-ফেল-করলেও পুরো-তালিকা চালিয়ে যায় (সম্পূর্ণ-চিত্র);
# মোট-সংখ্যা-অ্যাসার্ট (৩৩-সুইট) + নিজের-নাম-উপস্থিতি-অ্যাসার্ট;
...
SUITES=(s260-eventsfilter s261-resfilter s262-mufilter s263-mdfilter s264-adfilter s265-acfilter \
        ...
        s290-comboad s291-railcap)
if [ "${#SUITES[@]}" != "33" ]; then echo "FATAL: তালিকা-মোট ${#SUITES[@]} (৩৩-প্রত্যাশিত)"; exit 1; fi
...
FROM="${CHUNK_FROM:-1}"
TO="${CHUNK_TO:-33}"
if ! [ "$FROM" -ge 1 ] 2>/dev/null || ! [ "$TO" -le 33 ] 2>/dev/null || [ "$FROM" -gt "$TO" ]; then echo "FATAL: চাঙ্ক-সীমা-অবৈধ ($FROM..$TO)"; exit 1; fi
TOTAL=0; BAD=()
