#!/bin/bash
# s290-regression.sh — session290 পূর্ণ-রিগ্রেশন রানার (s260..s290 + role-policy + guard:design + audit:views)
# রীতি: প্রতি-সুইট-পরে ফল-সারাংশ; কোনো-এক-সুইট-ফেল-করলেও পুরো-তালিকা চালিয়ে যায় (সম্পূর্ণ-চিত্র);
# সরাসরি-তালিকা-সম্পাদনা (sed-ডেরিভেশন-গোটচা-প্রতিকার) + মোট-সংখ্যা-অ্যাসার্ট (৩২-সুইট) + নিজের-নাম-উপস্থিতি-অ্যাসার্ট
# ⚠️ ব্যাকগ্রাউন্ড-জব-গোটচা (session289): টুল-কল-ছেদে মারে — এ-রানার সামনের-কলেই চালান অথবা চাঙ্কে।
...
SUITES=(s260-eventsfilter s261-resfilter s262-mufilter s263-mdfilter s264-adfilter s265-acfilter \
        ...
        s290-comboad)
if [ "${#SUITES[@]}" != "32" ]; then echo "FATAL: তালিকা-মোট ${#SUITES[@]} (৩২-প্রত্যাশিত)"; exit 1; fi
...
if [ "$TOTAL" = "32" ] && [ "${#BAD[@]}" = "0" ]; then echo "ALL GREEN ✓"; else printf 'ব্যর্থ-তালিকা: %s\n' "${BAD[*]}"; exit 1; fi
