#!/bin/bash
# s295-regression.sh — session295 পূর্ণ-রিগ্রেশন রানার (s260..s295 + role-policy + guard:design + audit:views)
# রীতি: প্রতি-সুইট-পরে ফল-সারাংশ; কোনো-এক-সুইট-ফেল-করলেও পুরো-তালিকা চালিয়ে যায় (সম্পূর্ণ-চিত্র);
# মোট-সংখ্যা-অ্যাসার্ট (৩৭-সুইট) + নিজের-নাম-উপস্থিতি-অ্যাসার্ট;
...
if [ "${#SUITES[@]}" != "37" ]; then echo "FATAL: তালিকা-মোট ${#SUITES[@]} (৩৭-প্রত্যাশিত)"; exit 1; fi
...
FROM="${CHUNK_FROM:-1}"
TO="${CHUNK_TO:-37}"
if ! [ "$FROM" -ge 1 ] 2>/dev/null || ! [ "$TO" -le 37 ] 2>/dev/null || [ "$FROM" -gt "$TO" ]; then echo "FATAL: চাঙ্ক-সীমা-অবৈধ ($FROM..$TO)"; exit 1; fi
TOTAL=0; BAD=()
