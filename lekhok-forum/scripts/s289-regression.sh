#!/bin/bash
# s289-regression.sh — session289 পূর্ণ-রিগ্রেশন রানার (s260..s289 + role-policy + guard:design + audit:views)
# রীতি: প্রতি-সুইট-পরে ফল-সারাংশ; কোনো-এক-সুইট-ফেল-করলেও পুরো-তালিকা চালিয়ে যায় (সম্পূর্ণ-চিত্র);
# সরাসরি-তালিকা-সম্পাদনা (sed-ডেরিভেশন-গোটচা-প্রতিকার) + মোট-সংখ্যা-অ্যাসার্ট (৩১-সুইট) + নিজের-নাম-উপস্থিতি-অ্যাসার্ট
...
if [ "${#SUITES[@]}" != "31" ]; then echo "FATAL: তালিকা-মোট ${#SUITES[@]} (৩১-প্রত্যাশিত)"; exit 1; fi
case "${SUITES[*]}" in *s289-calpanel*) ;; *) echo "FATAL: রানারে s289-calpanel-অনুপস্থিত"; exit 1;; esac
TOTAL=0; BAD=()
for S in "${SUITES[@]}"; do
  TOTAL=$((TOTAL+1))
...
if [ "$TOTAL" = "31" ] && [ "${#BAD[@]}" = "0" ]; then echo "ALL GREEN ✓"; else printf 'ব্যর্থ-তালিকা: %s\n' "${BAD[*]}"; exit 1; fi
