...
| 05 | B-ফিড | কার্সার-ইনফিনিট-স্ক্রল | views/user/dashboard.ejs + routes/dashboard.js | 🟡 | সেন্টিনেল-ভিউপোর্ট ট্রিগার |
...
| 17 | D-লেখক | হোম-কিউরেশন সোশ্যাল-ফিল্টার | routes/pages.js (featured-সেকশন) | 🟡 | AVATAR/COVER_UPDATE-বাদ |
...
## Cross-Agent Note: Session 107 — যোগাযোগ-বার্তা ইনবক্স (পূর্ণাঙ্গ) (১৮ সেপ্টেম্বর ২০২৬) [নাম্বার-রেস: ১০৫×২+১০৬ অন্য-এজেন্টদের; সর্বোচ্চ+১ রীতিতে ১০৭]

**প্রেক্ষাপট:** session103-নোটের শীর্ষ-সুপারিশ — "contact_submissions-এর অ্যাডমিন-ইনবক্স-ভিউ (এখন পর্যন্ত সাবমিশন কেউ দেখে না!)"। /admin/messages ছিল খালি-লিস্ট (read/unread নেই, অ্যাকশন নেই, সার্চ/ফিল্টার/পেজিনেশন নেই, ইনলাইন-স্টাইল)।

**ইমপ্লিমেন্টেশন:**
- **db.js:** contact_submissions-এ `is_read`/`is_archived` (MIGRATION_SQL CREATE TABLE + LATER_COLUMNS — পুরনো লোকাল sql.js ও লাইভ Turso দুটোতেই বুটে কলাম পৌঁছায়, schema.sql-এ টেবিলটি নেই)
- **admin/routes.js:** GET /admin/messages পূর্ণাঙ্গ (q-সার্চ LIKE ×4-কলাম, f=all|unread|read|archived, পেজ 15/পেজ, counts105 স্ট্যাট-বান্ডল, `ORDER BY is_read ASC, id DESC` — অপঠিত আগে); POST /:id/read|unread|archive|unarchive|delete (delete = TA42.trashDelete → ট্র্যাশ+আন্ডু-টোস্ট ফ্রি); bulk-read/archive/unarchive (msgBulk105 হেল্পার) + BULK_TABLES-এ 'messages' → generic bulk-delete ফ্রি; সব রিডাইরেক্ট msgBack105 (f/q/page ধরে রাখে) + TA42.audit
- **server.js:** স্টাফ-লোকালস-ব্লকে `res.locals.unreadMsg105` (শুধু adminUser-সেশনে) — সাইডবার ব্যাজের উৎস
- **sidebar.ejs:** "বার্তা"-র পাশে .msg-badge105 (অপঠিত-সংখ্যা, >0 হলে); dashboard.ejs-এ বার্তা-কার্ডে .dash-unread105 ব্যাজ (counts.unreadMessages)
- **messages.ejs পূর্ণ-রিরাইট:** স্ট্যাট-স্ট্রিপ (মোট/অপঠিত/আজকের/আর্কাইভড) + ফিল্টার-পিল (কাউন্ট-ব্যাজ, অপঠিত=লাল) + পিল-সার্চ + বাল্ক-বার (sidebar-এর initBulkBar/injectCsrf42 চুক্তি) + কার্ড-লিস্ট (মোনোগ্রাম-টাইল tint-সাইকেল, অপঠিত=বাম-বর্ডার+গ্রেডিয়েন্ট+ডট, ২-লাইন clamp→এক্সপ্যান্ড টগল, সাবজেক্ট-চিপ, mailto-উত্তর Re:-সাবজেক্টসহ) + পেজিনেশন + প্রতি-ফিল্টার-এম্পটি-স্টেট + **কাস্টম কনফার্ম-মোডাল** (নেটিভ confirm() নিষিদ্ধ-নীতি; card-delete + bulkBar দুটোতেই)
- **admin.css:** session107-ব্লক EOF-অ্যাপেন্ড (.msg105/.pill105/.act-btn105/.cmodal105/.pg105/.msg-badge105/.dash-unread105 স্কোপড) + focus-visible + 640px + reduced-motion

**🚨 নতুন-গোটচা (আসল-ঘটনা থেকে):** `form.submit()` সাবমিটার-বাটনের `formaction` **অগ্রাহ্য** করে — ফর্মের ডিফল্ট action-এ যায়! বাল্ক-বারে একাধিক formaction-বাটন থাকলে কনফার্ম-মোডাল→submit() করলে ভুল এন্ডপয়েন্ট (bulk-read চাইতে গিয়ে bulk-delete!) চলে যায়। ফিক্স-প্যাটার্ন: কনফার্মের পরে বাটনেই `dataset.skipConfirm`-ফ্ল্যাগ দিয়ে আবার `.click()`। মেসেজ-ইনবক্সের 107-খ-ব্লকে রেফারেন্স-ইমপ্লিমেন্টেশন আছে।

**গোটচা-পুনঃপ্রমাণ:** চলমান সার্ভারের বাইরে (আলাদা node -e) lekhok.db-তে INSERT + saveDb → চলমান সার্ভারের পরবর্তী যেকোনো লেখায় (এমনকি UPDATE) তার স্টেল ইন-মেমরি স্টেট ডিস্কে flush হয়ে সিড-রো **মুছে যায়** — ডেমো/টেস্ট-ডেটা সর্বদা চলমান সার্ভারের **মধ্য দিয়ে** (HTTP POST) ঢোকান, সরাসরি DB-স্ক্রিপ্ট নয়।

**E2E (agent-browser, সব ✓):** ম্যাপ-সহ ১৪-পেজ স্মোক → ইনবক্স স্ট্যাট/পিল/কার্ড রেন্ডার; expand-টগল; পঠিত/অপঠিত টগল (কাউন্টার-সিঙ্ক); আর্কাইভ→আর্কাইভ-ভিউ→ইনবক্সে-ফেরত; card-delete→মোডাল→trashed=1+আন্ডু-টোস্ট; বাল্ক-সিলেক্ট→বাল্ক-রিড→saved-টোস্ট; বাল্ক-আর্কাইভ (re-click ফিক্স-পরে audit-প্রমাণিত bulk-archive); বাংলা-সার্চ→১-ম্যাচ+redirect-এ q-সংরক্ষণ; এম্পটি-স্টেট ×৩; সাইডবার+ড্যাশবোর্ড ব্যাজ; public /contact POST ×৩ → ইনবক্সে অপঠিত ৩ (ফুল-পাইপলাইন); 390px ওভারফ্লো-০; কনসোল-০; role-policy ৮০/১০৭ (২৭-ফেল = প্রাক-বিদ্যমান QA-সিড-স্টেট, PLANS-ডকুমেন্টেড — বার্তা-অসম্পর্কিত)।
