...
> ## 🚨 সবচেয়ে জরুরি সতর্কতা — Vercel/Turso মোড এখনো ভাঙা
> `db.js`-এ Turso/libsql সাপোর্ট যোগ করা হয়েছে (`vercel.json`, `@libsql/client`), কিন্তু
> **এখনো সম্পূর্ণ অকার্যকর।** পুরো অ্যাপে ৩২০+ জায়গায় পুরনো সিঙ্ক্রোনাস প্যাটার্ন
> (`db.prepare(sql).get()/.all()/.run()`) ব্যবহার হয় — `routes/*.js`, `admin/routes.js` সব
> জায়গায়। কিন্তু Turso ব্যাকএন্ডে এই মেথডগুলো **এখন ইচ্ছাকৃতভাবে error throw করে**
> ("use .getAsync()/.allAsync()/.runAsync() instead")। মানে **`TURSO_DATABASE_URL` সেট করে
> Vercel-এ deploy করলে সাইটের প্রায় প্রতিটা রিকোয়েস্ট ক্র্যাশ করবে।**
>
> **যা কাজ করে:** local dev / Railway / Render / VPS-এ `sql.js` মোডে (ডিফল্ট, `TURSO_DATABASE_URL`
> সেট না থাকলে) — এটা সম্পূর্ণ কার্যকর, এই ডকুমেন্টের বাকি সব টেস্ট এই মোডেই যাচাই করা।
>
> **Vercel-এ আসলে deploy করতে হলে যা লাগবে (এখনো করা হয়নি — এটা একটা বড়, আলাদা কাজ):**
> সব ৩২০+ কল-সাইটে `db.prepare(sql).get()` → `await db.prepare(sql).getAsync()` (ইত্যাদি)
> এ কনভার্ট করা, এবং প্রতিটা route handler-কে `async` বানানো। এই সেশনে হাত দেওয়া হয়নি —
> স্কোপ অনেক বড়, একটা আলাদা, পরিকল্পিত সেশনে করা উচিত।
