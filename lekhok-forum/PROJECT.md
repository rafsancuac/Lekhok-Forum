...
### সেশন ১১০ (১৮ সেপ্টেম্বর ২০২৬) — রোডম্যাপ-০৫ keyset-ইউনিয়ন-মার্জ + ১৭-যাচাই-সমাপ্তি + লোডিং-পলিশ + ইনডিপেন্ডেন্ট E2E (ক্রন-রিভিউ রাউন্ড)

**প্রসঙ্গ:** QA-সুইপ-প্রথম রাউন্ড — role-policy ১০৭/১০৭ + calls-E2E ৫৪/৫৪ + ব্রাউজার-সুইপ (হোম/ড্যাশ/মেসেঞ্জার/রিসোর্স-ডিটেইল, কনসোল-০, 390px-০) সব-গ্রিন; বাগ-না-পাওয়ায় মাস্টার-টেবিলের শেষ-২ আইটেমে (০৫ + ১৭) ফোকাস।

**সমান্তরাল-ইউনিয়ন-সিদ্ধান্ত:** রোডম্যাপ-০৫ (keyset-কার্সার) দুই-এজেন্টে স্বাধীনভাবে ইমপ্ল হয়েছিল — **session107-কমিট 9c53cab-এর ইমপ্ল ক্যানোনিকাল** (cursor=<ts>&cursorType&cursorId · ৩-কেস-শাখা-কন্ডিশন · p.id-as-id-অ্যালিয়াস · LIMIT+১-hasMore · ভাঙা-কার্সারে OFFSET-ফলব্যাক); আমার প্যারালাল-ইমপ্ল প্রত্যাহৃত, অনন্য-অবদান রক্ষিত। উভয়েই ২-SQL-গোটচা (JOIN-ambiguous ORDER BY id + UNION-compound নাম-কোয়ার্ক) স্বতন্ত্রভাবে আবিষ্কার করেছে — সমাধান-দুটি (অ্যালিয়াস + অর্ডিনাল-ORDER) PLANS-নোটে রেফারেন্স-সংরক্ষিত।

**এই-কমিটে রক্ষিত-অবদান:**
1. **ইনডিপেন্ডেন্ট E2E স্যুট (scripts/verify-session107-cursor.js, ২৩-চেক) — ক্যানোনিকাল-চুক্তিতে অ্যাডাপটেড:** ২৬-পোস্ট এক-সেকেন্ড-টাই-ব্যাচ সব-ঠিক-একবার · কার্সার-চেইন শেষ-পর্যন্ত ডুপ্লিকেট-শূন্য · **অ্যান্টি-ড্রিফট-প্রমাণ** (চেইন-মাঝে পোস্ট-সন্নিবেশে পুরনো-কার্সারের পরের-পেজ অপরিবর্তিত; OFFSET-এ একই-পরিস্থিতিতে পেজ-১-শেষ-আইটেম পুনরাবৃত্তি — কেন-কার্সার-প্রমাণ) · ভাঙা-কার্সার→graceful-OFFSET-ফলব্যাক (never-500) · ranked/গেস্ট/nextOffset-সামঞ্জস্য · ক্লিনআপ ২৭/২৭ (HTTP-API-ভিত্তিক — চলন্ত-সার্ভারে সরাসরি-DB-লেখা-নিষিদ্ধ)।
2. **০৫-লোডিং-পলিশ (dashboard.css session107-ব্লক):** সেন্টিনেল-লোডে দুই-স্তর স্কেলেটন-শিমার কার্ড + গ্লাস-স্টেট-পিল (backdrop-blur) + done-এমারল্ড-টিন্ট + reduced-motion/print-গার্ড।
3. **১৭-হোম-কিউরেশন-সোশ্যাল-ফিল্টার — যাচাই-সমাপ্তি:** সেশন-৯০-ই কঠোর-ফিল্টার (post_kind='writing' + shared_from IS NULL) ইমপ্ল করেছিল; লাইভ-যাচাই — হোমে AVATAR/COVER_UPDATE-লিক শূন্য; মাস্টার-টেবিলে ✅। **২০-আইটেম মাস্টার-টেবিল ২০/২০ সম্পূর্ণ।**

**যাচাই:** role-policy ১০৭/১০৭ ✓ calls-E2E ৫৪/৫৪ ✓ cursor-E2E ২৩/২৩ (ক্যানোনিকাল-চুক্তিতে) ✓ 390px×৪-পেজ ওভারফ্লো-০ ✓ কনসোল-০ ✓।

### সেশন ১০৭ (১৮ সেপ্টেম্বর ২০২৬) — রিসোর্স সিরিজ/সংকলন (প্লেলিস্ট) + ডিটেইল-স্টাফ-অ্যাকশন + স্যান্ডবক্স-লিংক-পোর্ট-সংরক্ষণ (ক্রন-রিভিউ রাউন্ড)

**প্রসঙ্গ:** session106-পরবর্তী স্টেবল-ফেজ — QA সুইপ ১৬/১৬ পেজ + role-policy 107/107 + কনসোল-০ → session106-সুপারিশের ①(সংকলন-সিরিজ) + ③(ডিটেইল-স্টাফ-অ্যাকশন) এই রাউন্ডে।

**নতুন ফিচার:**
1. **রিসোর্স সিরিজ/সংকলন (প্লেলিস্ট):** ① db.js LATER_COLUMNS += resources.series (TEXT ≤80) + series_order (INTEGER 1..999) — বুট-মাইগ্রেশন, idempotent ② /resources-এ সিরিজ-চিপ-রো (?series= সার্ভার-সাইড ফিল্টার, প্রতি-চিপে বাংলা-কাউন্ট, rsxExtra-প্যাটার্নে type/q/sort/series সব-ট্যাবে বহন; শেয়ারেবল-URL) ③ কার্ডে সিরিজ-এপব্যান্ড (কভার-বডির মাঝে গ্রেডিয়েন্ট-ব্যান্ড: সিরিজ-নাম + "N নং পর্ব" পিল, ?series= লিংক, data-search-এ সিরিজ-নাম) ④ ডিটেইল-পেজে সিরিজ-নেভিগেটর (সিরিজ-হেড + "পর্ব N/M" পিল + আগের/পরের-পর্ব কার্ড (শেষ/প্রথমে গার্ড-লেবেল) + সংখ্যাঙ্কিত পর্ব-তালিকা, বর্তমানে "চলছে"-হাইলাইট + aria-current; order→id ক্রম) + ব্যাজ-রো-তে সিরিজ-ব্যাজ (→?series=)।
2. **ডিটেইল-পেজে স্টাফ-অ্যাকশন (staffRole-ভিত্তিক):** অ্যাডমিন/সুপার → "এডিট" (/admin/resources/:id/edit) + "মুছুন" (fetch-POST ?_method=DELETE, X-CSRF-Token — ক্যাশেবল-পাবলিক-পেজের এফেমেরাল-টোকেন এড়াতে ?_u= fresh-GET থেকে মেটা-টোকেন রিফ্রেশ, session103 চুক্তি; confirm-ডায়ালগ + টোস্ট + /resources-এ রিডাইরেক্ট); মডারেটর → ম্যানেজ-লিংক (/moderator/resources)।
3. **স্টাফ-ফর্মে সিরিজ-ফিল্ড:** admin/resources/form.ejs + moderator-resources.ejs — সিরিজ-নাম (datalist-অটোকমপ্লিট, seriesNames107()) + পর্ব-ক্রম; INSERT/UPDATE-উভয়-পাথে স্যানিটাইজড (≤80 / 1..999)।
4. **🚨 স্যান্ডবক্স-লিংক-পোর্ট-সংরক্ষণ (সাইট-ওয়াইড ফিক্স):** গেটওয়ে-প্রিভিউতে (URL-এ XTransformPort থাকলে) layout.ejs-এ ক্যাপচার-ফেজ click-লিসেনার — সব অভ্যন্তরীণ <a href> (/path বা ?query)-এ পোর্ট-প্যারাম অটো-যোগ। **আগে query-only হ্রেফ (?category=… — session104 থেকে) ক্লিকে Next.js-ডিফল্ট-পোর্টে গিয়ে 404 হত** — সিরিজ-চিপ-টেস্টে ধরা পড়ে সাইট-ওয়াইড ফিক্স হয়েছে; লাইভে নো-অপ (পেজ-URL-এ প্যারাম নেই)।
5. **ডেমো-সিড:** scripts/seed-resources-107-series.js — ৩টি ডেমো-রিসোর্স "নবীন লেখক কর্মশালা" সিরিজে (পর্ব ১-৩); শুধু ফাঁকা-series রো, শিরোনাম-ম্যাচে — idempotent; ⚠️ সার্ভার-বন্ধ অবস্থায় চালাতে হয়।

**যাচাই (curl + node-byte + agent-browser @ gateway):** role-policy 107/107 ✓; ১৬-পেজ ম্যাট্রিক্স 200 ✓; ?series=→৩-কার্ড+active-চিপ ✓; ডিটেইল-নেভ (পর্ব ১/৩ ফার্স্ট-গার্ড, ২/৩ আগের+পরের, ৩/৩ লাস্ট-গার্ড, ৯/৫ ডুপ্লিকেট-অর্ডার) ✓; সিরিজ-ছাড়া রিসোর্সে নেভ-শূন্য ✓; চিপ-ক্লিক→ফিল্টার-পেজ (পোর্ট-সংরক্ষণ-ফিক্সে Next-404→৩-কার্ড) ✓; admin-INSERT seriesসহ→চিপ-কাউন্ট ৩→৫ ✓; testadmin-লগইনে এডিট+মুছুন-বাটন ✓ → মুছুন-ক্লিক→confirm→404 ✓; মডারেটর-ফর্মে ফিল্ড ✓; 390px-দুই-পেজ-ওভারফ্লো-০ ✓; কনসোল-০ ✓; epband-ভিজ্যুয়াল (স্ক্রিনশট) ✓।

**🚨 এনভায়রনমেন্ট-গোটচা (গুরুত্বপূর্ণ — QA-ইউজার-মিলিয়ে-যাওয়া-রহস্য):** `setsid env ... node server.js & echo $!` — setsid ফর্ক করলে $! ভুল-pid দেয় → pid-ফাইল-ভিত্তিক kill ব্যর্থ → **জম্বি-সার্ভার**। সার্ভার shutdown-এ db.flushDb() করে (server.js:900-913) → জম্বির পুরনো in-memory স্টেট SIGTERM-flush-এ ফাইল-ওভাররাইট করে (আমার series-সিড + QA-ইউজার এক-পর্ব মুছে গিয়েছিল)। **নিয়ম: সার্ভার-মারতে `pkill -f "node server.js"` (বা ss -tlnp দিয়ে pid); kill→৩সে-অপেক্ষা→তার-পরে-seed; seed-স্ক্রিপ্ট সবসময় সার্ভার-বন্ধ অবস্থায়।**

### সেশন ১১২ — 'পড়া চালিয়ে যান' ড্যাশবোর্ড-উইজেট + msx খসড়া-চিপ + ws91-হাইজিন

**প্রসঙ্গ:** রাউন্ড-৩ QA-সুইপ (১২-পেজ ২০০ + কনসোল-০ + guard:design গ্রিন) → বাগ-শূন্য প্রমাণিত → ফিচার-রাউন্ড।

**নতুন ফিচার:**
1. **'পড়া চালিয়ে যান' (continue-reading):** পাঠকের অসমাপ্ত-লেখা ড্যাশবোর্ড-সাইডবারে। article-reading.js এখন lf_read_pos-এ r/t-র পাশে ti(পরিষ্কার-টাইটেল, ' | লেখক ফোরাম'-সাফিক্স-বিহীন)+u(পাথ) রাখে; নতুন `assets/js/continue-reading.js` সর্বশেষ-৩ এন্ট্রির কার্ড আঁকে (dashboard.ejs-এ `#crxMount` + script-ইনক্লুড) — শূন্য-API (localStorage-মাত্র), খালি-তালিকায় কার্ড-আঁকা-হয়-না, প্রতি-সারিতে ৪px প্রগ্রেস-বার + বাংলা-% + ×-সরান-বাটন; লিঙ্কে-গেলে article-পেজের session৬৩-রিজিউম-ব্যানার স্ক্রল-অবস্থান ফেরায়। a11y: focus-visible, ×-এ aria-label; reduced-motion-সেফ; ≤640px-এ ২-লাইন-ক্ল্যাম্প।
2. **msx 'খসড়া (ড্রাফট)' চিপ:** ফুল-উইডথ ৫ম চিপ (`.msx-chip.wide{grid-column:1/-1}`) → /me?tab=drafts; routes/dashboard.js myStats-এ drafts-কাউন্ট (status='draft')।
3. **ws91-হাইজিন:** সেশন-১০৯-এ HTML flex-বারে পোর্ট-হওয়ার পরে SVG-যুগের মৃত-রুল (.ws91-chart, .ws91-bar, @keyframes ws91grow, .ws91-bar-num/-lbl — বেস+520px-মিডিয়া) সরানো; comment-stripped brace-depth ০/০ ব্যালেন্সড।
