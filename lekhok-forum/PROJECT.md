...
### সেশন ৩৬ (৭ সেপ্টেম্বর ২০২৬) — হটফিক্স: Vercel কোল্ড-বুট টাইমআউট (সাইট-ওয়াইড 500) — বুট ফাস্ট-পাথ + maxDuration

**ইউজার রিকোয়েস্ট**: "সব রিভেরিফাই কর!" — পুনঃযাচাইয়ের সময় লাইভ সাইট https://lekhok-forum.vercel.app **সব রুটে HTTP 500 (FUNCTION_INVOCATION_FAILED)** ফিরছিল।

- **নির্ণয় (প্রমাণসহ)**: স্ট্যাটিক `/assets/css/style.css` 200 (বিল্ড ঠিক), কিন্তু সব ফাংশন-রুট ~২০ সেকেন্ড পরে `x-vercel-error: FUNCTION_INVOCATION_FAILED` — অর্থাৎ বুটই শেষ হচ্ছিল না; ব্যর্থ ইনস্ট্যান্স প্ল্যাটফর্ম ফেলে দেওয়ায় প্রতিটি রিকোয়েস্ট নতুন কোল্ড বুটে গিয়ে মরত। Turso হোস্ট জীবিত (401 = শুধু অথ) ও অথেনটিকেটেড কোয়েরি ১৭৪ms-এ সফল — DB-আউটেজ নয়। আসল কারণ: **কোল্ড বুটে runMigrations + applyLaterMigrations + seed-চেক = ১০২টি সিরিয়াল কোয়েরি**; দূরবর্তী Turso-তে ~১৫৩ms/কোয়েরি হওয়ায় বুট **১৫.৬ সেকেন্ড** মেপা গেছে — lambda-init + require(server) যোগে ফাংশন-টাইমআউট ছাড়িয়ে যায়। সেশন ৩৩–৩৫-এ মাইগ্রেশন/কোড বাড়ায় সীমা ছাড়িয়েছে (আগে কোনাকুনি চলত)
- **ফিক্স ১ — বুট ফাস্ট-পাথ (db.js)**: পূর্ণ ইনিট সফল হলে `_boot_cache` টেবিলে `init_fingerprint` = md5(runMigrations, applyLaterMigrations, seedAdmin, seedIfEmptyLocal, seedDemoContent, ensureDemoModerator ফাংশন-সোর্স + BOOT_CACHE_VERSION) সেভ; পরের বুটে হ্যাশ মিললে সব মাইগ্রেশন/সিড-চেক স্কিপ। ফাংশন-সোর্স বদলালে হ্যাশ নিজেই বদলায় — নতুন মাইগ্রেশন সবসময় চলবেই। মেপা: বুট #১ (পূর্ণ) ১৫,৬৪৯ms → বুট #২ (ফাস্ট) **৬৪৮ms** (২৪×)
- **ফিক্স ২ — vercel.json**: `api/index.js`-এ `maxDuration: 60` — ভবিষ্যতের বড় মাইগ্রেশন-বুটেও হেডরুম
- **রিগ্রেশন**: লোকাল ফাইল-DB পথ অপরিবর্তিত (ফাস্ট-পাথ শুধু IS_TURSO-তে); test-lekhok.sh **77/77**, verify-session35-browser **27/27**, `node --check db.js` OK
- **শিক্ষা**: দূরবর্তী DB-তে প্রতি-বুট মাইগ্রেশন-চেক ব্যয়বহুল — স্কিমা-সংস্করণ ক্যাশ করা বাধ্যতামূলক; লাইভ-যাচাই সবসময় push-এর ঠিক পরে করা উচিত, পরে নয়
