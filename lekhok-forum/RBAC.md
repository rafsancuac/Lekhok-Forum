...
> **শেষ আপডেট:** সেশন ৮৩ (১৭ সেপ্টেম্বর ২০২৬)
...
## ০. রোল-হায়ারার্কি ও পোর্টাল-বিভাজন (সেশন ৮৩ — ইউজার-নির্দেশিত নীতি)

**স্তর:** সুপার এডমিন › এডমিন › মডারেটর › ইউজার (rank: 30/20/10/0 — `helpers/role-policy.js`)

| নীতি | বাস্তবায়ন |
|---|---|
| **আলাদা লগইন-পোর্টাল** | `/login` = শুধু সাধারণ ইউজার (স্টাফ/admin_users প্রত্যাখ্যাত, স্টাফ-পোর্টাল লিংকসহ); `/admin/login` = শুধু স্টাফ (superadmin/admin/moderator — users-টেবিলের স্টাফ এখান থেকেই লগইন করেন; সাধারণ ইউজার প্রত্যাখ্যাত, ইউজার-লগইন লিংকসহ) |
| **নিয়ন্ত্রণ-চেইন** | সুপার-এডমিন → এডমিন নিয়োগ/অপসারণ (`/admin/super/admins` + `/admin/users/:id/role`-এ admin/superadmin রোল = শুধু সুপার-অ্যাক্টর); এডমিন → মডারেটর/ইউজার (`/admin/moderators`, `/admin/users/:id/role`); মডারেটর → ইউজার-তদারকি (`/moderator/users` — user_mgmt স্কোপ, নিষেধ/ফেরত, রোল-পরিবর্তন নয়) |
| **সরাসরি-কানেকশন বন্ধ** | ইউজার↔মডারেটর, মডারেটর↔এডমিন, এডমিন↔সুপার-এডমিন — ১:১ DM (`GET/POST /messages/:username`) ও ফলো (`POST /follow/:id`) উভয়ই ব্লকড (উভয় দিক থেকে) — `rolePolicy.connectionBlocked()`; যোগাযোগ অফিসিয়াল চ্যানেলে (নোটিশ/অভিযোগ) |
| **সোয়াপ (শুধু নিজের ইউজার-আইডির সাথে)** | মডারেটর: `/moderator/switch` (প্যানেল↔ইউজার-ইন্টারফেস); এডমিন: `/admin/switch`; সুপার-এডমিন: কোনো ইউজার-মোড নেই। অন্য রোলের প্যানেলে সরাসরি সুইচ নেই (ক্রস-রোল 403 অক্ষুণ্ণ) |
| **নিয়োগ ছাড়া রোল-সুইচ নেই** | রোল কেবল ঊর্ধ্বতন-নিয়োগে বদলায়; আত্ম-উন্নীত/আত্ম-স্থানান্তর কোথাও সম্ভব নয় |

## ০-ক. গ্রুপ-সংযোজন নীতি ও তদারকি-নোটিফিকেশন (সেশন ৮৬-৯০)

**গ্রুপ-বাইপাস বন্ধ (সেশন ৮৬):** সরাসরি-কানেকশন ব্লক গ্রুপের মাধ্যমেও কার্যকর —
`POST /messages/group/create` ও `POST /messages/g/:id/members/add`-এ
`rolePolicy.connectionBlocked()` চেক: অ্যাক্টর↔টার্গেট adjacent জোড়া (user↔moderator,
moderator↔admin, admin↔superadmin) হলে সেই সদস্য গ্রুপে যুক্ত হয় না। সব-ব্লকড
নির্মাণে গ্রুপই তৈরি হয় না (`/messages?err=` নীতি-বার্তা); আংশিক-ব্লকে চ্যাটের
উপরে অ্যাম্বার নোটিশ (`?blocked=N`)। অর্থাৎ skip-level সংযোগ (যেমন user→admin)
গ্রুপে বৈধ, কিন্তু adjacent-জোড়া সংযোগ কোনো পথেই নয়।

**তদারকি-কারণ ও টার্গেট-বিজ্ঞপ্তি (সেশন ৯০):** মডারেটরের নিষেধ/ফেরতে (`POST
/moderator/users/:id/status`) ঐচ্ছিক `reason` (≤৩০০ অক্ষর, বেশি হলে `err=reason`)
— ① টার্গেট-ইউজার `notifications`-এ `type='moderation'` বিজ্ঞপ্তি পায় (শিরোণাম +
কারণ; `helpers/notify.js` → `notifyUser()`), ② `audit_log.detail`-এ
`moderator-oversight: <username> → <status> — কারণ: <reason>` সংরক্ষিত হয়।
বিজ্ঞপ্তি ব্যর্থ হলেও মূল অ্যাকশন ব্যর্থ হয় না (notify never-throws নীতি)।

**স্কোপ-প্রত্যাহার (সেশন ৯০):** সুপার-এডমিন নজরদারি-কার্ড থেকে
`POST /admin/super/moderators/:id/revoke-user-mgmt` — user_mgmt স্কোপ এক-ক্লিকে
প্রত্যাহার (রোল অপরিবর্তিত); মডারেটর সঙ্গে সঙ্গে `/moderator/users`-এ 403
(`requireScope` প্রতি-রিকোয়েস্ট DB-চেক)। পুনঃপ্রদান: `/admin/users/:id/scopes`।

****এক-স্তর-নিচের টপআপ-বাগফিক্স:** `ensureDemoModerator()` আগে `SELECT id` করত (role নয়) → `existing.role !== 'moderator'` প্রতি বুটে সত্য → প্রতি বুটে মডারেটরের কাস্টম-স্কোপ মুছে যেত। এখন role সহ সেলেক্ট — টপআপ শুধু সত্যিকারের খালি-অবস্থায়।
