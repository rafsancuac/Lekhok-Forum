...
router.post('/register', withUpload(avatarUpload), (req, res) => {
...
router.post('/profile/edit', withUpload(avatarUpload), (req, res) => {
  if (!req.session.user) return res.redirect('/login');
  ...
  const updated = db.prepare('SELECT id, username, full_name, avatar_url, gender, role FROM users WHERE id = ?').get(u.id);
